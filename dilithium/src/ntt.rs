...
const ZETAS: [i32; N::USIZE - 1] = {
    ...
    while idx < N::USIZE - 1 {
        ...
        twiddle *= reduce::MONT_R as i64;
        twiddle = reduce::leaktime::cmod(twiddle, Q as i64);
...
pub(crate) fn poly_ntt(p: &mut Poly32) {
    let mut twiddles = ZETAS.iter();

    for layer in 0..8 {
        let subpoly_len = 0x80 >> layer;
        for subpoly_offset in (0..N::USIZE).step_by(2 * subpoly_len) {
...
pub(crate) fn poly_invntt_tomont(p: &mut Poly32) {
    const MONT_R_I64: i64 = reduce::MONT_R as i64;
    const N_INV: i64 = reduce::leaktime::modinverse(N::I64, Q as i64);
    const F: i64 = reduce::leaktime::cmod(MONT_R_I64 * MONT_R_I64 * N_INV, Q as i64);
...
pub(crate) fn polyvec_ntt(vec: &mut [Poly32]) {
    for poly in vec.iter_mut() {
        poly_ntt(poly)
    }
}

pub(crate) fn polyvec_invntt_tomont(vec: &mut [Poly32]) {
    for poly in vec.iter_mut() {
